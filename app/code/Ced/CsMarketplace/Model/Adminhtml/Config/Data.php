...
class Data extends \Magento\Config\Model\Config
{
    ...
    )
    {
        ...
        if (isset($groupData['fields'])) {
            ...
            foreach ($groupData['fields'] as $fieldId => $fieldData) {
                ...
                $vendorId = ObjectManager::getInstance()->get('\Magento\Framework\Registry')->registry('data_com');
                ...
                if ($field && $field->getConfigPath()) {
                    ...
                    if (!empty($configPath) && strrpos($configPath, '/') > 0) {
                        ...
                        if (ObjectManager::getInstance()->get('Magento\Framework\Module\Manager')->isEnabled('Ced_CsCommission')) {
                            if ($vendorId) {
                ...
                $groupDatas = ObjectManager::getInstance()->get(Http::class)->getPost();
                $gcode = isset($groupDatas['group_code']) && strlen($groupDatas['group_code']) > 0 ? $groupDatas['group_code'] : (ObjectManager::getInstance()->get(Http::class)->getParam('gcode', false) ? ObjectManager::getInstance()->get(Http::class)->getParam('gcode') : '');
                if (strlen($gcode) > 0) {
                    $path = $gcode . '/' . $path;
                }
    ...
    {
        $is_csgroup = ObjectManager::getInstance()->get(Http::class)->getParam('is_csgroup');
    ...
    protected function _getConfig($full = true)
    {
        ...
        $groupData = ObjectManager::getInstance()->get(Http::class)->getPost();
        $gcode = isset($groupData['group_code']) && strlen($groupData['group_code']) > 0 ? $groupData['group_code'] : (ObjectManager::getInstance()->get(Http::class)->getParam('gcode', false) ? ObjectManager::getInstance()->get(Http::class)->getParam('gcode') : '');
        if (strlen($gcode) > 0) {
    ...
    protected function getConfigByPath($path, $scope, $scopeId, $full = true)
    {
        ...
        switch ($is_csgroup) {
            case 1:
                ...
                break;
            case 2 :
                $vendorId = ObjectManager::getInstance()->get(Http::class)->getParam('vendor_id', 0);
                $vendor = ObjectManager::getInstance()->get('Ced\CsMarketplace\Model\Vendor')->load($vendorId);
