...
class Vproducts extends \Ced\CsMarketplace\Model\FlatAbstractModel
{
      ...
      *
        ...
        if(!$this->isProductApprovalRequired() && $mode == self::NEW_PRODUCT_MODE) {
            $this->_objectManager->get('Ced\CsMarketplace\Helper\Mail')
                ->sendProductNotificationEmail(array($productId), self::APPROVED_STATUS);
        }
       return $this;
    }
        ...
        if (is_array($productIds)) {
            ...
            if (count($VproductCollection)>0) {
                ...
                foreach ($VproductCollection as $row) {
                    if ($row && $row->getId()) {
                        if(!$this->_objectManager->get('Ced\CsMarketplace\Helper\Data')->canShow($row->getVendorId()) && $checkstatus!= self::DELETED_STATUS) {
                            $errors['error'] = 1;
                            continue;
                        }
    ...
    public function getVendorProducts($checkstatus = '', $vendorId = 0, $productId = 0) 
    {
        ...
        if ($checkstatus === '') {
            $vproducts->addFieldToFilter('check_status', array('neq'=>self::DELETED_STATUS));
        } else {
