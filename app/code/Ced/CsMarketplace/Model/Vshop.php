    ...
    public function saveShopStatus(array $vendorIds, $shop_disable)
    {
        ...
        if (count($vendorIds) > 0) {
            foreach ($vendorIds as $vendorId) {
                ...
                $collection = $this->_objectManager->create('Ced\CsMarketplace\Model\Vproducts')->getVendorProducts('', $vendorId);
                foreach ($collection as $row) {
                    $productId = $row->getProductId();
                    if ($shop_disable == self::DISABLED) {
                        $this->_objectManager->create('Magento\Catalog\Model\Product\Action')->updateAttributes([$productId], ['status' => \Magento\Catalog\Model\Product\Attribute\Source\Status::STATUS_DISABLED], 0);
                        $this->_objectManager->create('Ced\CsMarketplace\Model\Vproducts')->load($productId, 'product_id')->setCheckStatus(\Ced\CsMarketplace\Model\Vproducts::NOT_APPROVED_STATUS)->save();
                    } 
                }
                $vendor = $this->_objectManager->get('Ced\CsMarketplace\Model\Vendor')->load($vendorId);
                $this->_objectManager->get('Ced\CsMarketplace\Helper\Mail')->sendShopEmail($shop_disable, '', $vendor);
            }
        }
