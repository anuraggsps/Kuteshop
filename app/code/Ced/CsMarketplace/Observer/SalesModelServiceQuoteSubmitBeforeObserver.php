                ...
                if ($quoteItem = $this->getQuoteItemById($orderItem->getQuoteItemId(), $this->quote)) {
                    ...
                    if ($additionalOptionsQuote = $quoteItem->getOptionByCode('additional_options')) {
                        ...
                        $additionalOptions = class_exists(
                            "\\Magento\\Framework\\Serialize\\Serializer\\Json")? $objectManager->create('\Magento\Framework\Serialize\Serializer\Json')->unserialize($additionalOptions) : unserialize($additionalOptions);
                    }
                    if ($quoteItem->getVendorId() && ($quoteItem->getProductType() !== Configurable::TYPE_CODE)){
                        $vendor = $objectManager->get('Ced\CsMarketplace\Model\Vendor')->load($quoteItem->getVendorId());
